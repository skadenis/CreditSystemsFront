...
<template>
  <div class="growl">
    <div
      class="growl__item growl__item--shown"
      :class="
        operation.status === 'ok'
          ? 'growl__item--success'
          : 'growl__item--error'
      ...
      @click="delete_mark(operation.id)"
      :key="key"
    >
      <i
        :class="
...
.growl__item {
  ...
  padding: 25px;
  display: flex;
  transition: margin 200ms ease, max-height 200ms, padding 250ms,
    transform 350ms 200ms cubic-bezier(0.175, 0.885, 0.32, 1.15),
